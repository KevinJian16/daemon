...
| MCP server | 方案 | 优先级 | 说明 |
|---|---|---|---|
| **google-calendar** | Google Calendar API MCP | P0 | navigator 场景时间轴、日程管理、训练安排 |
| **google-docs** | Google Docs API MCP | P0 | 文档读写，instructor 场景 assignment 核心 |
...
OpenWeatherMap 用途：navigator 根据天气决定室内/户外训练计划。与 intervals.icu 和 Google Calendar 联动。
...
- instructor 解释错误模式（为什么这样写不对，学术英文怎么表达）
...
Libraries.io 用途：engineer 检查依赖安全性和最新版本，autopilot 监控关注项目的依赖变更。
...
**LeetCode 刷题链路**（instructor 主导出题）：
1. instructor 根据用户薄弱点（Mem0 记录）通过 LeetCode MCP 搜索合适题目（tag/难度筛选）
2. instructor 出题："去做 LeetCode 第 X 题"，给出背景提示但不给答案
3. daemon 自动打开 VS Code（LeetCode 插件），用户在 Stage Manager 中安排窗口
4. 用户在 VS Code 中做题，代码存本地 `~/.leetcode/`
5. 做完后 instructor 通过 filesystem MCP 读用户代码 → 点评思路、时间/空间复杂度、代码风格
6. 如果卡住 → instructor 渐进式引导（先 hint → 再给思路框架 → 最后讲解）
7. Mem0 记录：已刷题目、薄弱知识点（DP/图/树/贪心等 tag）、常犯错误、进步曲线
8. instructor 基于 Mem0 调整出题策略，渐进式难度提升
...
**instructor SKILL.md 需新增**：`leetcode_coaching` skill — 搜索题库出题、引导式教学（先 hint 再深入）、读取本地解题文件点评、Mem0 记录刷题进度与弱项。
...
Obsidian vault 是用户的知识图谱（Markdown + 双向链接），存放在 Google Drive。daemon 的 Markdown 产出（报告/文章/笔记）写入 vault，二进制文件留 MinIO。写入 vault 的 agent：researcher（研究报告/文献笔记）、writer（文章草稿）、engineer（技术笔记/ADR）、instructor（学习笔记模板）。

---

## 3. 信息监控架构

daemon 不只是被动响应用户请求，还需要**主动获取信息**。这是 autopilot 和 researcher 的核心能力。
...
需新增（3 个 + DD-81）：
1. google-calendar — navigator 场景时间轴
2. google-docs — 文档读写
3. google-drive — 文件管理
...
**刷题**：
27. leetcode — instructor 主导出题 + 引导刷题（npm 社区包）
