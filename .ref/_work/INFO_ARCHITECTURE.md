...
| mentor | "正好，我们来看看这次 breaking change 的设计思路，你觉得为什么要这样改？" （老师，借机教学） |
| coach | "你这周的目标里有用到这个库的部分，需要调整计划吗？" （教练，关注执行计划） |
| operator | 已经自动检查了兼容性，如果没问题就不打扰用户 （自治，只在需要时汇报） |
...
- 跟 mentor 说"我想跟踪 NLP 领域最新论文" → 系统级订阅
- 跟 operator 说"每周给我一个技术周报" → 系统级定时任务
