...
#### 9.4 评价链机制（详见 DESIGN_QA.md Q7.5）

不存在独立评价表单。评价以**运行周期**为单位：一段评价 = 一次运行开始到下一次运行开始之间的所有对话（包含 running + settling）。评价链头 = Slip 的「执行」按钮，链尾 = 「收束」按钮。

- 每个运行周期边界触发洗信息压缩
- 收束时串联所有运行周期的压缩结果作为完整评价
...
评价以运行周期为单位（详见 DESIGN_QA.md Q7.5）。不存在独立评价表单。30min settling 窗口，超时后 custody 机制自动 close。
