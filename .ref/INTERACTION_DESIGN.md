...
对齐 Claude 网页端的视觉设计哲学，核心是**克制**。

- **内容至上**：UI chrome 几乎隐形。屏幕上能看到的几乎全部是内容。没有工具栏、状态栏、按钮组竞争注意力
- **排版即层级**：信息层次通过字号、字重、颜色深浅区分。不靠边框、背景色、图标
- **克制的色彩**：接近单色。颜色只在携带含义时出现（区分角色、标记状态），不做装饰
- **控件极少**：任何时刻可交互元素极少。不需要的操作不可见
- **列表干净**：列表项只展示最必要的信息。没有 badge 堆叠、图标阵列、操作按钮

### 0.2 动态：学 Apple

对齐 Apple 交互设计哲学，核心是**一致**。

- **全局统一的交互语法**：所有页面的交互方式完全相同。学一次，用所有地方。不存在"这个页面这样交互，那个页面那样交互"
- **层级导航**：列表 → 详情 → 子详情，始终 push/pop。用户永远知道自己在哪
- **渐进式披露**：列表只展示最少信息。详情通过导航进入，不在列表中内联展开。列表保持干净，详情保持完整
- **动效服务于理解**：所有状态转换有流畅过渡动画（spring/fade/morph），无瞬切。动效建立空间认知（"这个详情从那个条目展开，可以收回"），不是装饰
- **操作在详情中**：列表行里不放操作按钮。导航进条目后才看到能做什么。操作属于详情视图，不属于列表

### 0.3 可读性

人类信息处理能力有限，设计必须适配。

- **重复厌恶**：人对重复内容有天然厌恶感，超过 3 条结构相同的行注意力急剧下降。数据有天然分组时必须体现分组，不能平铺
- **信息合并**：强相关的信息合并到同一列或区域，减少视觉列数
- **上下文操作**：操作按钮只在当前状态下有意义时才显示，不全部铺开

### 0.4 可理解性

信息呈现在使用者的抽象层级，不是实现者的抽象层级。

- 数字需要上下文（"3/24 占用" 而非 "3"）
- 状态需要人类可读的含义（"健康" 而非原始状态码）
- 错误需要下一步指引，不是技术堆栈
- 不直接展示原始数据（JSON、日志原文、文件内容）
...
Console 的使用者是"修车工"：知道怎么观测和维修，不需要知道汽车的设计构造。所有交互模块化——拔插、开关、切换——不暴露系统文件和内部数据。

- **严格压缩交互**：原则上不出现文本输入框。不直接展示系统内文件内容。修车工做模块化操作，不需要看到引擎内部
- **统一交互语法**：所有页面遵循同一套交互模式——干净列表 → 统一方式展开详情 → 详情中操作。用户学会一个页面就会用所有页面
- **遵循 §0 全部设计准则**：静态学 Claude（克制、内容至上、控件极少），动态学 Apple（统一交互语法、层级导航、渐进式披露），可读性（分组、合并、上下文操作），可理解性（操作者抽象层级）
