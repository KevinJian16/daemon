...
### Q10.5 Console UI 设计标准

**Console 的使用者是"修车工"**（maintainer），不是系统设计者。修车工知道怎么观测和维修，不需要知道汽车的设计构造。Console 的交互必须模块化——拔插、开关、切换——不需要看到引擎内部。

**设计约束（详见 INTERACTION_DESIGN.md §0）：**

1. **严格压缩交互**：原则上不出现文本输入框。不直接展示系统内文件内容
2. **统一交互语法**：所有页面遵循同一套交互模式（列表 → 详情 → 操作），不是每个页面有自己的交互逻辑
3. **可读性**：人对重复内容天然厌恶（>3 条结构相同即触发）；数据有分组则必须体现分组；操作按状态上下文显示
4. **可理解性**：信息呈现在操作者的抽象层级，不是实现者的抽象层级。不展示原始 JSON/日志/文件
