...
### Q1.5.1 Instinct 的执行模型

Instinct 的规则执行必须是**代码层确定性执行**，不依赖 LLM 遵守指令。

| 规则类型 | 执行方式 | 例子 |
|---------|---------|------|
| 资源限制 | 代码硬卡 | Ration 上限、token 预算、并发数 |
| 内容过滤 | 正则/关键词 | 敏感词拦截、格式校验 |
| 流程约束 | 状态机 | 必须经过 arbiter 审查、不能跳过 settling |
| 数据完整性 | 校验函数 | Voice 写入前检查 token 上限、矛盾检测 |

`instinct.md` 的作用是给 agent 解释**为什么**有这些规则，让 agent 行为更合理。但规则本身的执行不靠 LLM 遵守 prompt。

### Q1.5.2 Instinct 是信息门控
...
不存在独立的评价表单。Deed 的 settling 阶段就是对话本身。

用户在 Deed 对话框中的发言（"太正式了""算了就这样吧"）即反馈内容。用户按下的按钮（执行 / 收束）即反馈动作。
...
按钮按下时触发洗信息。洗信息只做**机械提取**：

- Ledger：rework 次数、消息数、时长（客观数字）
- Voice 候选：关键词匹配提取，写入前必须用户确认

不存原文语义，不用 LLM 解读对话内容。

按钮决定洗出物路由：
- 按「执行」→ 洗出物喂给下次执行（Brief 补充）+ 系统（Ledger 统计）
- 按「收束」→ 洗出物只喂系统（无下次执行）

一套洗信息机制，按钮选路由。所有洗出物流入系统前必须过 Instinct 代码门控。
...
按钮 = 状态边界（结构控制），对话 = 边界间的语义内容。Slash command = 按钮。
