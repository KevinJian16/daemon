...
### Q7.3 反馈 = 对话（按钮是对话的快捷方式）

不存在独立的评价表单。用户在 Slip 统一对话流中的发言（"太正式了""算了就这样吧"）即反馈。按钮（执行 / 停止 / 收束）与对话在效果上完全等价——按钮只是常用操作的 UI 便利，不是独立的控制通道。
...
**评价链**：
- 链头 = 第一个运行周期开始（创建 Deed 并开始执行）
- 链尾 = 最后一个运行周期结束（Deed 收束）
...
Slip 和 Deed 在数据模型上是独立实体，但在 UI 上共享同一条对话流。Deed 作为内嵌执行块存在于 Slip 页面中，不是独立页面。状态和按钮各自独立，但对话统一在 Slip 层。
...
Slip 只有一条对话流，承载所有交互。对话内容按运行周期自然分段：

- **无 Deed 时**：对话用于调整 DAG。DAG 修改的结果体现在 DAG 本身，不洗给系统学习。
- **Deed 运行期间**：对话用于执行调整和评价反馈。

### Q12.5 洗信息触发 = 运行周期边界

洗信息在运行周期边界触发（见 Q7.5），不由按钮触发。按钮和对话在效果上等价。
...
按钮、拖拽等 UI 操作与对话输入在效果上完全等价——按钮只是常用操作的 UI 便利形式。洗信息的分段依据是运行周期边界（见 Q7.5），不是按钮按下。

所有非对话操作在对话流中生成自然语言记录（见 Q7.6），确保对话流是完整的行为日志。
...
15. Slip 和 Deed 各有独立对话框（对话统一在 Slip 层，Deed 是内嵌执行块）
16. 按钮和对话是不同控制通道（按钮是对话的 UI 便利形式，效果完全等价）
17. 洗信息由按钮触发（洗信息在运行周期边界触发）
