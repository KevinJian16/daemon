...
**claude_code / codex 适用场景与机制**：
- `claude_code`：reviewer 审查对外发布内容、复杂 Project 初始规划、admin 自愈修复
- `codex`：engineer 遇到复杂实现时
- 两者均通过 Temporal Activity subprocess 调用，**绕过 OC，不需要 session 管理**
- 调用前 Activity 自动注入必要上下文（MEMORY.md 内容 + 对应 skill 内容），context 保持精准最小
...
| `mentor` | 学习指导 | daemon 引导，用户学习 | 学习、探索、接受评价 | 学习进度、理解水平、教学经验 |
...
| `engineer` | 编码与技术任务（实现功能、调试、工具链） | fast | 代码风格、技术决策偏好 |
| `writer` | 写作与内容生产（论文、文章、报告、对外文本） | creative | 写作风格、格式偏好 |
| `reviewer` | 质量审查（事实校验、逻辑一致性、风格合规） | review | 质量标准、常见失败模式 |
| `publisher` | 对外发布、消息整理、通知 | fast | 渠道格式、发布风格 |
| `admin` | 体检、诊断、自愈、暖机主导（Stage 3+）、skill 管理 | analysis | 系统基线、体检历史、参数校准记录 |
...
┌────────────────────────────────────────────────────────────────────┐
│                          用户界面                                    │
│  桌面客户端（Electron）：4 个场景对话 + 场景 panel + 浏览器 view     │
...
| 层级 | Agent | 默认模型 | 理由 |
|---|---|---|---|
| L1 | copilot / mentor / coach / operator | fast（对话）/ analysis（规划） | 对话快响应，DAG 设计需推理 |
| L2 | researcher | Qwen Max（analysis） | 搜索+深度分析，质量优先 |
| L2 | engineer | MiniMax M2.5（fast） | 编码通用，speed 优先 |
| L2 | writer | GLM Z1 Flash（creative） | 写作创意，文风优先 |
| L2 | reviewer | Qwen Max（review） | 审查严谨，质量优先 |
| L2 | publisher | MiniMax M2.5（fast） | 发布动作，speed 优先 |
| L2 | admin | Qwen Max（analysis） | 系统评估需要判断力 |
| — | — | 智谱 embedding-3 | Embedding |

**[DEFAULT]** 模型名称和 provider 绑定保持配置化，不在正文硬编码到具体厂商 SKU。上表记录当前选择，具体模型表见参考文档附录 B 和运行时配置。
...
**FINAL 规则：客户端提供三种展示模式，4 个场景各自独立对话。**

**原则：专业工具外部调起，轻量展示 panel 内嵌，客户端只做调度和聚合。**

| 模式 | 技术 | 用途 |
|---|---|---|
| **对话** | 纯文本 | 和 L1 agent 交流（4 个独立对话，类似微信多个聊天） |
...
**技术选型**：Electron（Chromium 内核）。浏览器 view 需要打开任意网页，Chromium 兼容性最好且与用户日常浏览器（Chrome）一致。前端代码同时用于桌面客户端和远程 Web 访问（见 §6.10）。
...
**[DEFAULT]** 对话流传输方案：WebSocket（双向）或 SSE（服务端推送）+ POST（客户端发送）。WebSocket 开销小（每帧 2-14 字节头），SSE 更简单。具体选型在实现阶段决定。对话流消息可携带多种类型（文本、panel 更新指令、浏览器导航指令等），客户端根据类型分发到对应 view。

### 4.10 Telegram

**FINAL 规则：4 个独立 Bot Token，4 个独立 DM。**

用户在 Telegram 里看到 4 个联系人（copilot / mentor / coach / operator），各自 DM。和桌面客户端的 4 个聊天页面完全同步。场景之间无协作关系，不放在同一个群里。

Telegram 承载：
- 完成 / 失败 / 告警通知
- 品味类确认请求的推送（用户可直接在 Telegram 回复）
- 移动端完整对话交互（与桌面客户端同步）
...
**FINAL 规则：桌面客户端是用户与 daemon 交互的主入口。** 三个 view 见 §4.2。

| 组件 | 说明 |
|---|---|
| 桌面应用 | Electron（Chromium 内核），同一套前端代码 |
| 菜单栏图标 | 状态指示（绿/黄/红）+ 点击打开主窗口 + 右键菜单（启停/系统信息） |
| 菜单栏右键菜单 | Start / Stop daemon、今日任务数、本周体检状态、打开 Langfuse / Temporal UI（面向 CC/admin 的管理入口） |

菜单栏图标与桌面应用是同一个进程。菜单栏常驻，主窗口按需打开/关闭。

#### 6.10.3 远程访问

**FINAL 规则：daemon 可从任意设备远程使用，无需在远程设备上安装 VPN。**

FINAL 规则：方案为 Tailscale Funnel + OAuth 认证（见 §6.13.1）。

```
本地 Mac：  Electron 桌面 app → localhost:port（OAuth 登录，token 持久化）
远程设备：  浏览器/轻壳 app → https://daemon.tailnet.ts.net → Tailscale Funnel → localhost:port
                                        ↑ OAuth 登录（Google / GitHub）
```

- **服务端**：daemon API 进程（FastAPI）同时 serve 前端静态文件 + API
- **Tailscale Funnel**：将本地服务暴露为 HTTPS URL，免费，端到端加密，无需开端口
- **认证**：OAuth（Google / GitHub），本地和远程统一使用，登录后 JWT token 持久化
- **手机端**：浏览器直接访问 HTTPS URL，或做成 PWA（加到主屏幕）。如需更原生体验，可做 WebView 轻壳 app（Swift/Kotlin）
- **前端同构**：桌面 Electron app 和远程浏览器使用完全相同的前端代码
...
| Skill 文件（openclaw/workspace/） | git 管理 | 每次变更 | 永久 |
...
| 场景 | 认证方式 |
|---|---|
| 本地 Electron 客户端（localhost） | OAuth 登录，token 持久化，不需要每次登录 |
...
|---|---|---|
| agent | 是 | OC session 加载 agent workspace 下的 skills/ 目录，agent 自行匹配 |
...
**AGENTS.md** 保留为通用行为规范（session 启动、memory 管理、安全边界）。**TOOLS.md** 保留为工具清单 + 约定。
