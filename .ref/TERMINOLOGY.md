...
| 英文 canonical name | 中文显示名 | 定义 |
|---|---|---|
| `Psyche` | 心智 | 系统总心智层，由下列组件构成 |
| `Instinct` | 本能 | 系统不可覆盖的硬规则，代码层确定性执行（`InstinctEngine`） |
| `Voice`（画像） | 画像 | 身份与写作风格文件（`psyche/voice/`），与 §2.3 的 Voice（对话服务）不同 |
| `Preferences` | 偏好 | 用户可调整的系统偏好（`PsycheConfig` TOML） |
| `Rations` | 配给 | 资源额度与配额配置（`PsycheConfig` TOML，与 §2.3 Ration 同源） |
| `Ledger` | 账簿 | 机械统计（dag_templates / folio_templates / skill_stats / agent_stats，SQLite） |
| `SourceCache` | 源缓存 | 外部知识的 TTL 缓存层（embedding 检索） |

> **已废除**：旧 `Memory`（记忆）和 `Lore`（阅历）不再是正式 Psyche 组件。外部知识由 `SourceCache` 承担，经验学习改为 `Ledger` 机械统计。
...
|---|---|---|
| `pulse` | 脉察 | 基础健康巡检例行 |
| `record` | 记录 | 把行事结果写回 Ledger 统计的例行 |
| `witness` | 见证 | 观察趋势、提取信号的例行 |
| `focus` | 聚焦 | 调整系统关注重点的例行 |
| `relay` | 转递 | 负责快照与上下文转递的例行 |
| `tend` | 照料 | 负责清理、提交、轮转与维护的例行 |
| `curate` | 策藏 | 负责归档与藏库整理的例行 |

> **已废除**：旧 `learn`（学习）和 `distill`（提炼）已删除。学习机制改为 Ledger 机械统计，由 `record` 和 `witness` 承担。
