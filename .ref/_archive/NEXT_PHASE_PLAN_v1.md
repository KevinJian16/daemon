...
| §13 归档清理 + Drive（memory）| ✅ DONE | _cold_export_memory / _upload_to_drive / _cleanup_local_jsonl 均实现 |
| §13 用户产出上传 Drive（outcomes）| ❌ GAP | outcomes/ 目录结构已定义（v2 §13.4.1），代码未实现 |
...
### 1.1 §15.2 Telegram 通知（已确认实现，可跳过）

`spine/routines.py:1726` `_notify_skill_evolution_digest()` 已实现阈值检查 + Telegram 推送。状态更新为 ✅ DONE。

### 1.2 §13 用户产出上传 Drive（outcomes）

**目标**：任务完成后，将双语产出文件上传到 Google Drive `daemon/outcomes/YYYY-MM/YYYY-MM-DD HH:MM <任务标题>/`。

**规则**（来自 v2 §13.4.1）：
- 目录名含时间（永不冲突），文件名仅含标题（不带时间）
- 零内部 ID 出现在路径中
- 评分、元数据等不上传；仅上传双语 Markdown 产出文件
- outcome index 维护 `task_id → drive_path` 映射

**实现位置**：`temporal/activities.py` 中任务完成 step，或 `spine/routines.py` 新增 `_upload_outcome_to_drive(task_id, title, files)`。
