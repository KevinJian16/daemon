...
| Tailscale | ❌ DISABLED |
| Circuit（持续执行回路）| ✅ DONE（cron 触发；待重命名 chains→circuits） |
...
| 任务 | 类型 | 验收点 |
|---|---|---|
| 总结一个 URL | Pulse | events.jsonl 有 run_completed |
...
Circuit 是 Daemon 第四种执行单元（Pulse / Thread / Campaign / **Circuit**）。调度容器，持有模板 + 触发策略，循环产出任务实例直到断路。详见 V2 §22。

**已实现（cron 触发）：**
- `GET/POST/PUT/DELETE /circuits`（代码中暂为 `/chains`，待重命名）
- `POST /circuits/{id}/trigger`：手动立即触发
- Telegram：列表命令 + 取消回路命令
- Console Schedules 面板：Circuit 区块

**待完成：**
- 代码从 chains 重命名为 circuits
