...
Push model (system → user):
  ...
  Events: run_started, run_completed, milestone_done, campaign_plan_ready,
          run_failed, run_paused, skill_evolution_digest.
...
# Pending rating store: {callback_prefix: run_id} — e.g. "rate:abc123" → run_id
# Also tracks which run_ids are awaiting optional comment after rating.
_awaiting_comment: dict[int, dict] = {}  # {user_id: {"run_id": ..., "score": ...}}
...
def _rating_keyboard(run_id: str) -> list[list[dict]]:
    """4-choice inline keyboard matching Portal quick rating."""
    choices = [("超出预期", 5), ("满足需求", 4), ("偏浅了", 2), ("不满意", 1)]
    return [
        [{"text": label, "callback_data": f"rate:{run_id}:{v}"} for label, v in choices[:2]],
        [{"text": label, "callback_data": f"rate:{run_id}:{v}"} for label, v in choices[2:]],
    ]
...
def _retry_cancel_keyboard(run_id: str) -> list[list[dict]]:
    """Build a retry/cancel inline keyboard for failed runs."""
    return [[
        {"text": "↺ 重试", "callback_data": f"run_retry:{run_id}"},
        {"text": "✗ 取消", "callback_data": f"run_cancel:{run_id}"},
    ]]
...
def _handle_callback_query(cq: dict) -> None:
    ...
    if action == "rate" and len(parts) >= 3:
        run_id, score_str = parts[1], parts[2]
        try:
            score = int(score_str)
        except ValueError:
            score = 3
        ...
        try:
            httpx.post(
                f"{DAEMON_API}/runs/{run_id}/feedback",
                json={"source": "telegram", "quick_rating": score},
                timeout=15,
            )
        except Exception as exc:
            logger.warning("Failed to submit rating for run %s: %s", run_id, exc)
        # Store for optional comment.
        _awaiting_comment[user_id] = {"run_id": run_id, "score": score}
    ...
    if action == "campaign_confirm" and len(parts) >= 2:
        ...
        try:
            ...
            _send_message(chat_id, f"✓ Campaign 已确认，运行已启动：`{data_resp.get('run_id', campaign_id)}`")
        except Exception as exc:
    ...
    # ── Run retry ──────────────────────────────────────────────────────
    if action == "run_retry" and len(parts) >= 2:
        run_id = parts[1]
        try:
            ...
            pass
        try:
            resp = httpx.post(f"{DAEMON_API}/runs/{run_id}/retry", json={}, timeout=20)
            resp.raise_for_status()
            _send_message(chat_id, f"✓ 运行 `{run_id}` 已重新提交。")
        except Exception as exc:
            _send_message(chat_id, f"✗ 重试失败：{str(exc)[:200]}")
        return

    # ── Run cancel ─────────────────────────────────────────────────────
    if action == "run_cancel" and len(parts) >= 2:
        run_id = parts[1]
        try:
            ...
            pass
        try:
            resp = httpx.post(f"{DAEMON_API}/runs/{run_id}/cancel", json={}, timeout=20)
            resp.raise_for_status()
            _send_message(chat_id, f"✓ 运行 `{run_id}` 已取消。")
        except Exception as exc:
...
def _handle_update(update: dict) -> None:
    ...
    if text == "/start" or text == "/help":
        ...
        _send_message(chat_id,
            ...
            "`/status` — 查看运行中的执行\n"
            "`/outcomes` — 最近的产出\n"
            "`/pause` — 暂停当前执行（等当前步骤完成后暂停）\n"
            "`/reset` — 重置会话\n"
            "`/circuits` — 查看周期 Circuit\n"
            "`取消Circuit <circuit_id>` — 取消某个周期 Circuit\n"
            "`/help` — 此帮助\n\n"
    ...
    if text == "/pause":
        _log_telegram_event("command", {"chat_id": chat_id, "user_id": user_id, "cmd": text})
        try:
            r = httpx.get(f"{DAEMON_API}/runs?run_status=running&limit=1", timeout=10)
            runs = r.json() if r.ok else []
            if not runs:
                _send_message(chat_id, "当前没有运行中的执行。")
            else:
                run = runs[0]
                run_id = run.get("run_id", "")
                resp = httpx.post(f"{DAEMON_API}/runs/{run_id}/pause", json={}, timeout=15)
                resp.raise_for_status()
                _send_message(chat_id, f"⏸ 运行 `{run_id}` 暂停请求已发送，等待当前步骤完成后暂停。")
        except Exception as exc:
    ...
    if text == "/status":
        _log_telegram_event("command", {"chat_id": chat_id, "user_id": user_id, "cmd": text})
        try:
            r = httpx.get(f"{DAEMON_API}/runs?run_status=running&limit=10", timeout=10)
            runs = r.json() if r.ok else []
            if not runs:
                _send_message(chat_id, "No runs currently active.")
            else:
                lines = [f"*{len(runs)} active run(s):*"]
                for r in runs:
                    lines.append(f"• `{r.get('run_id') or ''}` — {r.get('title') or r.get('run_type') or '?'}")
                _send_message(chat_id, "\n".join(lines))
        except Exception as e:
    ...
    if text == "/circuits":
        _log_telegram_event("command", {"chat_id": chat_id, "user_id": user_id, "cmd": "circuits"})
        try:
            r = httpx.get(f"{DAEMON_API}/circuits", timeout=10)
            circuits = r.json() if r.ok else []
            if not circuits:
                _send_message(chat_id, "暂无周期 Circuit。")
            else:
                lines = [f"*周期 Circuit ({len(circuits)}):*"]
                for c in circuits:
                    status = c.get("status", "?")
                    icon = "✅" if status == "active" else "⏸" if status == "paused" else "❌"
                    lines.append(f"{icon} `{c['circuit_id']}` — {c.get('name','')} [{c.get('cron','')}] 已跑 {c.get('run_count',0)} 次")
                _send_message(chat_id, "\n".join(lines))
        except Exception as e:
            _send_message(chat_id, f"Error: {e}")
        return
    ...
    if text == "/outcomes":
        ...
        try:
            ...
            if not items:
                ...
                for o in items:
                    ts = str(o.get("delivered_utc") or "")[:10]
                    lines.append(f"• {o.get('title') or o.get('run_id')} — {ts}")
                _send_message(chat_id, "\n".join(lines))
        except Exception as e:
    ...
    # ── Circuit cancel ────────────────────────────────────────────────────────────
    _circuit_cancel_prefixes = ("取消circuit ", "取消Circuit ", "cancel circuit ", "/cancel_circuit ")
    if any(text.lower().startswith(p.lower()) for p in _circuit_cancel_prefixes):
        _log_telegram_event("command", {"chat_id": chat_id, "user_id": user_id, "cmd": "cancel_circuit"})
        circuit_id = ""
        for prefix in _circuit_cancel_prefixes:
            if text.lower().startswith(prefix.lower()):
                circuit_id = text[len(prefix):].strip()
                break
        if not circuit_id:
            _send_message(chat_id, "用法：取消Circuit <circuit_id>")
        else:
            try:
                resp = httpx.delete(f"{DAEMON_API}/circuits/{circuit_id}", timeout=10)
                if resp.status_code == 200:
                    _send_message(chat_id, f"✓ Circuit `{circuit_id}` 已取消，不再自动触发。")
                else:
                    _send_message(chat_id, f"✗ 取消失败：{resp.text[:200]}")
            except Exception as exc:
                _send_message(chat_id, f"✗ 取消失败：{str(exc)[:200]}")
        return
    ...
    if user_id in _awaiting_comment and not text.startswith("/"):
        ...
        try:
            ...
            _log_telegram_event("feedback_comment_ok", {"chat_id": chat_id, "run_id": run_id})
            _send_message(chat_id, "✓ 评语已补充。")
        except Exception as exc:
            logger.warning("Failed to submit feedback comment for run %s: %s", run_id, exc)
...
async def webhook(request: Request):
    ...
    if text == "/confirm" and user_id and user_id in _pending_plans:
        ...
        try:
            ...
            _log_telegram_event("confirm_ok", {"chat_id": chat_id, "user_id": user_id, "run_id": result.get("run_id", "")})
            _send_message(chat_id, f"✓ 运行已提交。Run ID: `{result.get('run_id')}`")
        except Exception as e:
...
async def notify(request: Request):
    ...
    Events handled:
      run_started        → 收到运行确认 + 规模
      run_completed      → 完整摘要 + 1-5 评分 inline keyboard
      milestone_done      → milestone 摘要 + 1-5 评分 inline keyboard
      campaign_plan_ready → 计划摘要 + 确认/取消 inline keyboard
      run_failed         → 错误说明 + 重试/取消 inline keyboard
      run_paused         → 暂停通知
    ...
    if event == "run_started":
        ...
        title = str(payload.get("title") or payload.get("run_type") or "运行")
        scale = str(payload.get("work_scale") or "thread")
        ...
        _send_inline(chat_id, msg, _rating_keyboard(run_id))
        ...
        run_id = str(payload.get("run_id") or campaign_id)
        ...
        msg = f"❌ *{title}* 失败\n\n{error[:600]}"
        _send_inline(chat_id, msg, _retry_cancel_keyboard(run_id))

    elif event == "run_paused":
        run_id = str(payload.get("run_id") or "")
        title = str(payload.get("title") or "运行")
        _send_message(chat_id, f"⏸ *{title}* 已暂停 (`{run_id}`)\n发送任意消息或 /resume 继续。")
