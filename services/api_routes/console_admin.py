...
def register_console_admin_routes(app: FastAPI, *, ctx: Any) -> None:
    ...
    def console_dashboard():
        ...
        return {
            ...
            "running_deeds": sum(1 for row in deeds if str(row.get("deed_status") or "") == "running"),
            "settling_deeds": sum(1 for row in deeds if str(row.get("deed_status") or "") == "settling"),
            "failed_24h": sum(1 for row in deeds if str(row.get("deed_sub_status") or "") == "failed" and _is_within_24h(str(row.get("updated_utc") or row.get("failed_utc") or ""))),
            "active_folios": sum(1 for row in folios if str(row.get("status") or "") == "active"),
            "parked_folios": sum(1 for row in folios if str(row.get("status") or "") == "active" and str(row.get("sub_status") or "") == "parked"),
            "active_slips": sum(1 for row in slips if str(row.get("status") or "") == "active"),
            "open_drafts": sum(1 for row in ctx.folio_writ.list_drafts() if str(row.get("status") or "") == "drafting"),
