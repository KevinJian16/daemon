...
from runtime.semantic import SemanticFingerprint, SemanticMappingError


def resolve_semantic(dispatch, request: dict) -> SemanticFingerprint:
    if fp_raw := request.get("semantic_fingerprint"):
        if isinstance(fp_raw, dict):
            return dispatch._semantic.from_fingerprint_dict(fp_raw)
    ...
    if task_type := str(request.get("task_type") or "").strip():
        return dispatch._semantic.from_task_type(task_type, title=str(request.get("title") or ""))
